...
# Each path component of urdf_relpath must consist only of alphanumerics,
# underscores, hyphens, and periods — no shell metacharacters or quotes.
...
def _validate_package_name(name: str) -> None:
    ...
    leading digits).
    """
    if not ROS_PACKAGE_NAME_RE.fullmatch(name):
        raise ValueError(
...
def _validate_urdf_relpath(p: Path) -> None:
    """Raise ValueError if *p* is absolute, contains '..' segments, or any
    path component contains unsafe characters (shell metacharacters, quotes,
    whitespace, etc.)."""
    if p.is_absolute():
    ...
    for part in p.parts:
        if not SAFE_PATH_COMPONENT_RE.fullmatch(part):
            raise ValueError(
                f"urdf_relpath component {part!r} contains unsafe characters; "
                f"each segment must match {SAFE_PATH_COMPONENT_RE.pattern}"
            )
...
<package format="3">
  ...
  <maintainer email={maintainer_email_attr}>{maintainer_name}</maintainer>
  <license>MIT</license>
    ...
    *,
    ...
    (out_dir / "package.xml").write_text(
        PACKAGE_XML.format(
            ...
            maintainer_name=_xml_escape(maintainer_name),
            maintainer_email_attr=_xml_quoteattr(maintainer_email),
        )
    )
